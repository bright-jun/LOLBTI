...
1. Frontend
	-	Vue
		-   store에 저장한 소환사 정보가 새로고침하면 유지되지 않은 문제가 발생했음 
		-   이를 해결하기 위해 동적 라우팅을 사용. 
		-   라우터를 이용해 라우팅의 주소를 설정할 때 query string으로 소환사 이름을 함께 설정함
		-   새로고침 하더라도 parameter에서 소환사 이름을 가지고 검색이 가능함.
	-	Vuetify
		-   vuetify를 이용해 navbar, banner, footer 등을 구현했고 grid system으로 구조를 쉽고 깔끔하게 구성할 수 있었음.
		-   UI의 구성요소들이 반응형으로 동작했지만 grid 내 구성요소들이 반응형으로 동작하지 않는 문제점이 발견됨.
		-   추후 과제로 남김.
	-	chartjs
		-	통계 시각화
		-   js 파일을 통해 차트를 렌더링하는 방식으로, 컴포넌트에서 data를 정리해 props값으로 넘겨주었다. 
		-   data 형식을 정확히 인지하지 못했을 때 차트가 정확히 표현되지 않는 일이 많았다.
		-   vuechart.js 와 chart.js가 비슷한듯 다른 요소들이 많았고 기호에 맞게 차트를 표현하기에 많은 어려움이 있었다.
		-   chart.js의 properties(속성)을 적용해보는 식으로 진행했지만 그 마저도 한계가 있었다. 다른 차트 표현 라이브러리가 있는지 알아보아야겠다.
...
-	⭐⭐API KEY 제약사항 및 해결과정⭐⭐
	...
	-	아이템 추천을 하기 위해서 30만개의 데이터를 수집 했음. 총 3.5GB가량 되는 양. 최소 30만개의 requests요청해야했음.
	-	💥300000 / 100 * 2 min = 6000 min = 100 hour = 4일밤낮 넘게 돌려야 겨우 받아올 수 있음.
	>	❕ API key를 여러개를 확보하여, requests제한에 걸렸을때 key를 바꾸면서 요청하면 괜찮지 않을까?!
	-	API key를 8개를 확보했다
	-	이론상 300000 / 100 * 2 / 8 = 725 min = 12 hour!
	...
	-	최종적으로는 10개의 API key를 사용하여 30만개의 데이터를 수집하였음
	-	10 * 100 (requests/2min)
	-	300000 / 1000 * 2 = 600 min = 10 hour
	-	온전히 public key 만으로 production key의 1/3의 효율성으로 데이터를 수집할 수 있게 되었음
	-	실제로는 20시간이상이 걸림
		-	key 바꿔가면서 써야했음
		-	💥중간중간 error생김
		-	requests뿐만 아니라 코드가 돌아가는 시간도 포함됨
