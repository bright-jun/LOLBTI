    ...
    <v-row>
      <v-col cols="6">
        <v-card class="text-center" style="padding: 10px;">
          ...
          <v-img :src="`http://ddragon.leagueoflegends.com/cdn/10.19.1/img/champion/${myChamp.id}.png`" style="margin: auto; width: 100px;"></v-img>
          <p style="margin-top: 10px; fontSize: 15px;">{{myChamp.name}}{{myChamp.key}}</p>
          <v-autocomplete class="mt-5" v-model="myChampName" outlined hide_details
                      :items="champList"
                      label="내 챔피언"
                    ></v-autocomplete>
          ...
          >
            <v-row class="mx-1">
              <v-col v-for="champAvatar in champAvatarList" :key="champAvatar" class="d-flex child-flex" cols="2" style="padding: 5px;">
                <v-img :src="`http://ddragon.leagueoflegends.com/cdn/10.19.1/img/champion/${champAvatar}.png`" aspect-ratio="1" class="grey lighten-2" @click="selectAvatar(champAvatar)"></v-img>
              </v-col>
            </v-row>
          </v-virtual-scroll>
        </v-card>
      ...
      <v-col cols="6">
        <v-card class="text-center" style="padding: 10px;">
          ...
          <v-img :src="`http://ddragon.leagueoflegends.com/cdn/10.19.1/img/champion/${vsChamp.id}.png`" style="margin: auto; width: 100px;"></v-img>
          <p style="margin-top: 10px; fontSize: 15px;">{{vsChamp.name}}{{vsChamp.key}}</p>
          <v-autocomplete class="mt-5" v-model="vsChampName" outlined hide_details
                      :items="champList"
                      label="상대 챔피언"
                    ></v-autocomplete>
          <v-virtual-scroll :items="items"
          ...
          >
            <v-row class="mx-1">
              <v-col v-for="champAvatar in champAvatarList" :key="champAvatar" class="d-flex child-flex" cols="2" style="padding: 5px;">
                <v-img :src="`http://ddragon.leagueoflegends.com/cdn/10.19.1/img/champion/${champAvatar}.png`" aspect-ratio="1" class="grey lighten-2" @click="selectVsAvatar(champAvatar)"></v-img>
              </v-col>
            </v-row>
          </v-virtual-scroll>
        </v-card>
